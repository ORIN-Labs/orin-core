# ORIN AI Agent Guide (LangChain + Anthropic)
...
- LLM inference with Claude 3.5 Sonnet (`@langchain/anthropic`)
- Strict JSON parsing (`JsonOutputParser`)
- Runtime schema validation
- Deterministic SHA-256 generation for hash-lock
- Voice synthesis output via ElevenLabs
...
- Sends `raw_response` text to ElevenLabs API.
...
- ElevenLabs HTTP failures are surfaced with status + body.
